...
class BankingApplicationWithTransfer {
    ...
    private static void deposit(Scanner scanner) {
        ...
        System.out.print("Enter the amount to deposit: LK");
        double amount = scanner.nextDouble();

        if (amount > 0) {
            accounts.put(accountNumber, accounts.getOrDefault(accountNumber, 0.0) + amount);
            System.out.println("Deposit successful. Current balance: LK" + accounts.get(accountNumber));
        } else {
    ...
    private static void withdraw(Scanner scanner) {
        ...
        System.out.print("Enter the amount to withdraw: LK");
        double amount = scanner.nextDouble();
    ...
    private static void checkBalance(Scanner scanner) {
        ...
        System.out.println(
                "Current balance for account " + accountNumber + ": LK" + accounts.getOrDefault(accountNumber, 0.0));
    }
    ...
    private static void transferFunds(Scanner scanner) {
        ...
        System.out.print("Enter the amount to transfer: LK");
        double amount = scanner.nextDouble();
        ...
        if (amount > 0 && amount <= accounts.getOrDefault(sourceAccount, 0.0)) {
            ...
            System.out.println("Current balance for " + sourceAccount + ": LK" + accounts.get(sourceAccount));
            System.out.println("Current balance for " + destinationAccount + ": LK" + accounts.get(destinationAccount));
        } else if (amount <= 0) {
            System.out.println("Invalid amount. Please enter a positive value.");
        } else {
